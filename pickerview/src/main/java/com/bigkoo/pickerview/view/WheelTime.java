...
public class WheelTime {
    ...
    public WheelTime(View view, boolean[] type, int gravity, int textSize,List<String> h ,
                     List<String> m ,List<String> s ,boolean is12Hour) {
        super();
        this.view = view;
        this.type = type;
        this.gravity = gravity;
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        if(this.hourData != null ){
            wv_hours.setAdapter(new ArrayWheelAdapter(this.hourData));
            wv_hours.setCurrentItem(0);
        }else {
            if(this.is12Hour){
                wv_hours.setAdapter(new NumericWheelAdapter(0, 11));
                if(h > 11){
                    wv_hours.setCurrentItem(h -12);
                }
            }else {
        ...
        if(this.minuteData != null){
            wv_minutes.setAdapter(new ArrayWheelAdapter(this.minuteData));
        ...
        if(this.secondData != null){
            wv_seconds.setAdapter(new ArrayWheelAdapter(this.secondData));
...
//                maxItem = endD;
    ...
    public String getTime() {
        ...
        if(hourData !=null || minuteData != null|| secondData != null){
            sb.append(hourData != null ? hourData.get(wv_hours.getCurrentItem()) :type[3]? wv_hours.getCurrentItem()+":":"")
                    .append(minuteData != null ? minuteData.get(wv_minutes.getCurrentItem()) : type[4] ? wv_minutes.getCurrentItem()+" ":"")
                    .append(secondData != null ? secondData.get(wv_seconds.getCurrentItem()) : "");
        }else {
